...
class GeofenceMetricSpace(MetricSpace):
    ...
    def distance(self, x1: np.ndarray, x2: np.ndarray) -> np.ndarray:
        ...
        if x1.ndim == 1 and x2.ndim == 1:
            ...
            # Historically we supported reshaping grids into the distance function
            # but test-suite and higher-level adapters expect that 3D grids
            # passed to a geofence metric are treated as unsupported here and
            # should raise an explicit error to avoid accidental broadcasting
            # or large memory allocations. Raise ValueError so callers can
            # handle grid evaluation at a different abstraction level.
