...
--    No OLAP, menos JOINS = Mais Performance. Dimensões pré-calculadas evitam recálculo
--    a cada query.
--    O Modelo A (OLTP Normalizado) exigiria joins em cadeia
--    (Pedido -> Cliente -> Cidade -> Estado) custosos para leitura.
...
-- ==============================================
-- a) **Snowflake:** Não. A tabela Categoria estaria ligada à Tabela Produto
--    (Sub-dimensão), exigindo dois saltos (JOINS) até a Fato.
--    (Fato -> Produto -> Categoria)
--
-- b) **Star Schema (Simplicidade).**
--    Ferramentas de BI funcionam melhor com modelos de esquema estrela direto
--    (One-Hop Join).
--    Snowflake exige que a ferramenta de BI gerencie hierarquias complexas de joins,
--    o que pode reduzir performance e intuitividade para o usuário final.
