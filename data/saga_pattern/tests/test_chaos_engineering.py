...
from sage.outbox.types import OutboxEvent, OutboxStatus, OutboxConfig
from sage.outbox.storage.memory import InMemoryOutboxStorage
from sage.outbox.brokers.memory import InMemoryBroker
from sage.outbox.brokers.base import BrokerConnectionError, BrokerPublishError
from sage.storage.memory import InMemorySagaStorage
from sage.storage.base import SagaStorageConnectionError
...
class TestWorkerCrashRecovery:
    ...
    async def test_multiple_workers_no_duplicate_processing(self):
        ...
        workers = [
            OutboxWorker(storage, broker, OutboxConfig(batch_size=3))
            for _ in range(3)
        ]
...
class TestDatabaseConnectionLoss:
    ...
    async def test_database_connection_loss_during_saga_execution(self):
        ...
        async def failing_load(saga_id):
            if connection_lost:
                raise SagaStorageConnectionError("Connection lost")
...
class TestBrokerDowntime:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(reason="Worker doesn't retry within single process_batch - requires external retry loop")
    async def test_broker_connection_failure_exponential_backoff(self):
        ...
        assert succeeded, f"Failed after {max_attempts} attempts with {attempt_count} publish attempts"
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(reason="Worker behavior - marks events FAILED, doesn't auto-retry in same batch")  
    async def test_broker_publish_timeout(self):
        ...
        async def slow_publish(event):
            ...
            if attempt_count == 1:
                ...
                raise BrokerPublishError("Timeout")
    ...
    async def test_partial_batch_failure(self):
        ...
        async def selective_fail(event):
            ...
            if event_num % 2 == 1:  # Odd numbers fail
                raise BrokerPublishError("Publish failed")
...
class TestConcurrentFailures:
    ...
    async def test_database_and_broker_both_fail(self):
        ...
        async def unreliable_save(*args, **kwargs):
            if not db_healthy:
                raise SagaStorageConnectionError("Database down")
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(reason="Worker doesn't implement cascading retry logic - requires external orchestration")
    async def test_cascading_failure_recovery(self):
        ...
        assert attempt >= failure_count, f"Expected at least {failure_count} attempts, got {attempt}"
    ...
    async def test_no_data_loss_under_failures(self):
        ...
        async def random_failure(event):
            if random.random() < 0.3:  # 30% failure rate
                raise BrokerPublishError("Random failure")
            return await original_publish(event)
        ...
        for i in range(num_events):
            ...
            assert event is not None
            assert event.status == OutboxStatus.SENT, \
                f"Event {i} not published: {event.status}"
