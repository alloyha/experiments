...
class TestFailureAndCompensation:
    ...
    async def test_partial_compensation_failure(self):
        ...
        async def failing_comp(result, ctx):
            ...
            raise ValueError("Compensation 2 failed")
...
class TestDAGCompensation:
    ...
    async def test_dag_parallel_compensation(self):
        ...
        await saga.add_step(
            "step2", 
            action, 
            await make_comp("comp2", 0.3),
            dependencies={"step1"}
        )
        await saga.add_step(
            "step3", 
            action,
            await make_comp("comp3", 0.3),
            dependencies={"step1"}
        )
...
class TestSagaSequential:
    ...
    async def test_sequential_saga_with_compensation(self):
        ...
        async def step3_action(ctx: SagaContext):
            execution_order.append("step3")
            raise Exception("Step 3 intentional failure")
        ...
        await saga.add_step("step3", step3_action, max_retries=0)  # No retries, just initial attempt
...
class TestSagaParallel:
    ...
    async def test_dag_with_dependencies(self):
        ...
        await saga.add_step("calculate_total", calculate_total_action, dependencies={"fetch_user", "fetch_products"})
...
class TestSagaIntegration:
    ...
    async def test_ecommerce_order_processing(self):
        ...
        await saga.add_step("reserve_inventory", reserve_inventory_action, dependencies={"validate_order"})
        await saga.add_step("validate_payment", validate_payment_action, dependencies={"validate_order"})
        await saga.add_step("charge_payment", charge_payment_action, dependencies={"reserve_inventory", "validate_payment"})
    ...
    async def test_financial_trade_execution(self):
        ...
        await saga.add_step("execute_trade", execute_trade_action, dependencies={"validate_funds", "check_market_hours", "get_current_price"})
        await saga.add_step("update_portfolio", update_portfolio_action, dependencies={"execute_trade"})
        ...
        assert saga.execution_batches[0] == {"validate_funds", "check_market_hours", "get_current_price"}
...
class TestEdgeCases:
    ...
    async def test_exception_in_compensation_doesnt_stop_other_compensations(self):
        ...
        async def comp2_fails(result, ctx):
            ...
            raise ValueError("Comp 2 failed")
...
class TestConcurrency:
    ...
    async def test_concurrent_saga_execution_protected(self):
        ...
        try:
            ...
            assert False, "Expected saga to reject second execution attempt"
            ...
            assert ("already executing" in str(e).lower() or 
                    "can't start" in str(e).lower() or
                    "transition" in str(e).lower()), f"Unexpected error: {e}"
    ...
    async def test_orchestrator_handles_concurrent_submissions(self):
        ...
        results = await asyncio.gather(
            *[orchestrator.execute_saga(s) for s in sagas]
        )
...
class TestPerformance:
    ...
    async def test_overhead_is_minimal(self):
        ...
        # Should complete very quickly (< 100ms for 10 no-op steps)
        assert duration < 0.1
