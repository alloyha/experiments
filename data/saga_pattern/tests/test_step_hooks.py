...
# We'll import from sage.decorators once implemented
# from sage.decorators import Saga, step, compensate, StepHook
    ...
    async def test_on_compensate_called_during_compensation(self):
        ...
        class TestSaga(Saga):
            ...
            async def charge_payment(self, ctx):
                raise ValueError("Payment failed")  # Triggers compensation
...
class TestHookHelperDecorators:
    ...
    async def test_on_step_enter_decorator(self):
        ...
        from sage.decorators import Saga, step
        from sage.hooks import on_step_enter
    ...
    async def test_publish_on_success_helper(self):
        ...
        from sage.decorators import Saga, step
        from sage.hooks import publish_on_success
        from sage.outbox.storage.memory import InMemoryOutboxStorage
        ...
        class OrderSaga(Saga):
            @step(
                ...
                on_success=publish_on_success(
                    storage=storage,
                    event_type="order.created",
                    payload_builder=lambda ctx, result: {
