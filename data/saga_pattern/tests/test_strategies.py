...
class MockDAGStep:
    ...
    async def execute(self):
        """Mock step execution"""
        if self.cancelled:
            raise asyncio.CancelledError("Step was cancelled")
            
        await asyncio.sleep(self.duration)
        self.executed = True
        
        if self.should_fail:
            raise SagaStepError(f"Step {self.name} failed intentionally")
    ...
    def test_strategy_factory_pattern(self):
        ...
        def create_strategy(strategy_type: str, **kwargs):
            if strategy_type == "fail_fast":
                return FailFastStrategy()
            elif strategy_type == "wait_all":
                return WaitAllStrategy()
            elif strategy_type == "fail_fast_grace":
