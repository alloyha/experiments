...
class TestSagaListenerBasic:
    ...
    async def test_listener_receives_step_failure_event(self):
        ...
        class TestListener(SagaListener):
            async def on_step_failure(self, saga_name: str, step_name: str, ctx: dict, error: Exception):
                received_events.append({
                    "event": "step_failure",
                    "step_name": step_name,
                    "error": str(error)
    ...
    async def test_listener_receives_saga_failed_event(self):
        ...
        class TestListener(SagaListener):
            async def on_saga_failed(self, saga_name: str, saga_id: str, ctx: dict, error: Exception):
                received_events.append({
                    "event": "saga_failed",
                    "error": str(error)
                })
...
class TestBuiltInListeners:
    ...
    async def test_metrics_listener_records_execution(self):
        ...
        from sage.decorators import Saga, step
        from sage.listeners import MetricsSagaListener
    ...
    async def test_outbox_listener_publishes_events(self):
        ...
        from sage.decorators import Saga, step
        from sage.listeners import OutboxSagaListener
