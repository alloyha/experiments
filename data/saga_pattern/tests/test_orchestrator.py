...
class TestSagaOrchestrator:
    ...
    async def test_mixed_success_and_failure(self, orchestrator: SagaOrchestrator):
        ...
        tasks = [
            orchestrator.execute_saga(success_saga),
            orchestrator.execute_saga(failure_saga)
        ]
    ...
    async def test_orchestrator_concurrent_execution(self, orchestrator: SagaOrchestrator):
        ...
        # With concurrent execution and 0.01s delay each, should take around 0.01-0.02s, not 0.03s
        assert end_time - start_time < 0.05  # Should be much faster than sequential
