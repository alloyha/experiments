...
from sage.outbox.brokers.factory import create_broker_from_env, get_available_brokers
from sage.outbox.worker import OutboxWorker, OutboxConfig
...
class TestCoverageImprovements:
    ...
    def test_broker_factory_from_env(self, monkeypatch):
        ...
        # Test Kafka (Mocking import/class)
        # We assume KafkaBroker is available in this env since we installed it
    ...
    async def test_kafka_broker_idempotency(self):
        ...
        # Mock AIOKafkaProducer at the class level or module level
        with patch("sage.outbox.brokers.kafka.AIOKafkaProducer") as MockProducer:
    ...
    async def test_rabbitmq_broker_idempotency(self):
        ...
        # Mock connect implementation to avoid real networking
        original_connect = broker.connect
        
        # We need to test the logic guarding _connected
        # But we can't easily execute the robust connection logic without mocks
        # So we just mock the internal call
        
        with patch("aio_pika.connect_robust", new_callable=AsyncMock) as mock_connect:
            mock_connect.return_value = AsyncMock()
            mock_connect.return_value.channel.return_value = AsyncMock()
    ...
    async def test_compensation_graph_missing_dependency(self):
        ...
        from sage.compensation_graph import SagaCompensationGraph, MissingDependencyError
    ...
    async def test_compensation_graph_circular_dependency(self):
        ...
        from sage.compensation_graph import SagaCompensationGraph, CircularDependencyError
    ...
    async def test_decorator_saga_compensation_error(self):
        ...
        from sage.decorators import Saga, step, compensate
    ...
    async def test_storage_factory_postgresql(self):
        ...
        from sage.storage.factory import create_storage
    ...
    async def test_core_saga_add_step_while_executing(self):
        ...
        from sage.core import Saga as ClassicSaga
        from sage import SagaContext, SagaExecutionError
        import asyncio
