...
Measure and optimize Sage outbox worker performance.
...
kubectl scale deployment outbox-worker --replicas=0 -n sage

# Apply benchmark job
kubectl apply -f k8s/benchmark-job.yaml

# Watch logs
kubectl logs -f job/outbox-benchmark -n sage
```
...
kubectl top pods -n sage

# Check worker logs for errors
kubectl logs -n sage -l app=outbox-worker --tail=100 | grep -i error
```
