...
sequenceDiagram
    ...
    User->>kubectl: create namespace sage
    ...
    subgraph Pod["outbox-worker-xxx-yyy"]
        subgraph Container["worker container"]
            ...
            subgraph Probes["Health Probes"]
                LIVE[Liveness: python -c import sage]
                READY[Readiness: python -c import sage]
            end
        end
...
|------|-------------------|
| `secrets-local.yaml` | sage-db-credentials, sage-broker-credentials |
| `configmap.yaml` | sage namespace, outbox-worker-config |
