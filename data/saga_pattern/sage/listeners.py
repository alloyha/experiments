...
Example:
    >>> from sage import Saga, step
    >>> from sage.listeners import SagaListener, MetricsSagaListener
    >>> 
...
class SagaListener(ABC):
    ...
    async def on_saga_complete(
        self, saga_name: str, saga_id: str, ctx: Dict[str, Any]
    ) -> None:
...
class LoggingSagaListener(SagaListener):
    ...
    def __init__(self, logger_instance: Optional[logging.Logger] = None, level: int = logging.INFO):
...
class MetricsSagaListener(SagaListener):
    ...
    Integrates with sage.monitoring.metrics for consistent metrics collection.
...
class TracingSagaListener(SagaListener):
    ...
    Integrates with sage.monitoring.tracing for OpenTelemetry support.
    ...
    def __init__(self, tracer=None):
        if tracer is None:
            from sage.monitoring.tracing import saga_tracer
            tracer = saga_tracer
        self.tracer = tracer
