...
Usage:
    >>> from sage.outbox.brokers import RedisBroker, RedisBrokerConfig
    >>> 
...
class RedisBroker(BaseBroker):
    ...
    async def publish(
        ...
        try:
            ...
            # Add key if provided
            if key:
                fields[b"key"] = key.encode() if isinstance(key, str) else key

            # Add headers
            if headers:
                for h_key, h_value in headers.items():
                    field_name = f"header:{h_key}"
                    fields[field_name.encode()] = (
                        h_value.encode() if isinstance(h_value, str) else str(h_value).encode()
                    )
            ...
            message_id = await self._client.xadd(
                ...
                approximate=True,  # ~ MAXLEN for efficiency
            )
