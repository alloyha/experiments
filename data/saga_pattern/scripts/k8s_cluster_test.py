#!/usr/bin/env python3
"""
Kubernetes Cluster Test Script for Sage Outbox Pattern

This script tests the Sage outbox worker cluster with multiple scenarios:
...
Prerequisites:
    - kubectl port-forward -n sage svc/postgresql 5432:5432
...
class OutboxTester:
    """Test harness for the Sage outbox pattern."""
    ...
    async def insert_event(
        ...
        async with self.pool.acquire() as conn:
            ...
            """,
                ...
                event_type,
                json.dumps(payload),
                json.dumps({"source": "k8s-test", "timestamp": datetime.now(timezone.utc).isoformat()}),
            )
...
async def test_bulk(tester: OutboxTester):
    ...
    if processed:
        success(f"✓ Bulk test PASSED - Processed {events_processed} events in {process_time:.2f}s ({events_processed/process_time:.0f} events/sec)")
        return True
    else:
...
async def main():
    parser = argparse.ArgumentParser(
        description="Kubernetes Cluster Test Script for Sage Outbox Pattern",
