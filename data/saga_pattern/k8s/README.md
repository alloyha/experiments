# Kubernetes Deployment Manifests for Sage Saga Pattern

This directory contains production-ready Kubernetes manifests for deploying the Sage Saga Pattern system.
...
kubectl logs -f deployment/outbox-worker -n sage
```
...
```bash
# Manual backup
kubectl exec -it postgresql-0 -n sage -- \
  pg_dump -U postgres saga_db > backup.sql

# Restore
kubectl exec -i postgresql-0 -n sage -- \
...
velero backup create sage-backup --include-namespaces sage

# Restore
velero restore create --from-backup sage-backup
```
...
kubectl exec -it deployment/outbox-worker -n sage -- \
...
kubectl logs -f postgresql-0 -n sage
...
- Check logs: `kubectl logs -f deployment/outbox-worker -n sage`
- Check metrics: `kubectl port-forward service/outbox-worker-metrics 8000:8000 -n sage`
