...
```bash
# Immediate scale up
kubectl scale deployment/outbox-worker -n sage --replicas=10

# Check status
kubectl get hpa -n sage outbox-worker-hpa
```
...
### View Worker Logs
```bash
kubectl logs -f -n sage -l app=outbox-worker --tail=100
```
...
### View Recent Errors
```bash
kubectl logs -n sage -l app=outbox-worker --since=10m | grep -i error
```
