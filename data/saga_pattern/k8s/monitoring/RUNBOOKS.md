# Operational Runbooks - Sage Saga Pattern

This directory contains operational runbooks for troubleshooting and resolving common issues with the Sage saga orchestration system.
...
1. **Check failure distribution:**
   ...
   kubectl logs -n sage -l app=saga-orchestrator --tail=1000 | grep "status=failed"
...
3. **Review recent deployments:**
   ```bash
   kubectl rollout history deployment -n sage
   ```
...
4. **Check downstream service health:**
   ...
   # Check service logs
   kubectl logs -n sage -l app=<failing-service>
   ```
...
kubectl set env deployment/saga-orchestrator -n sage PAUSE_SAGA_<TYPE>=true

# Scale down if needed
kubectl scale deployment/saga-orchestrator -n sage --replicas=1
```

**Recovery:**
```bash
# Resume execution
kubectl set env deployment/saga-orchestrator -n sage PAUSE_SAGA_<TYPE>=false

# Scale back up
kubectl scale deployment/saga-orchestrator -n sage --replicas=3
```
...
1. **Identify stuck sagas:**
   ```bash
   # Query database
   kubectl exec -n sage postgresql-0 -- psql -U sage -c \
...
3. **Review orchestrator logs:**
   ```bash
   kubectl logs -n sage -l app=saga-orchestrator | grep "<saga_id>"
   ```
...
```sql
-- Connect to database
kubectl exec -it -n sage postgresql-0 -- psql -U sage
...
```bash
kubectl rollout restart deployment/saga-orchestrator -n sage
```

**Increase timeouts (if pattern persists):**
```yaml
# Update configmap
kubectl edit configmap saga-config -n sage
...
1. **Identify failed compensations:**
   ```bash
   # Check logs
   kubectl logs -n sage -l app=saga-orchestrator | grep "compensation.*failed"
...
3. **Check compensating service:**
   ```bash
   kubectl logs -n sage -l app=<service> --tail=100
   ```
...
1. **Check pod status:**
   ```bash
   kubectl get pods -n sage -l app=outbox-worker
   kubectl describe pods -n sage -l app=outbox-worker
   ```

2. **Check recent events:**
   ```bash
   kubectl get events -n sage --sort-by='.lastTimestamp' | grep outbox-worker
   ```

3. **Review logs:**
   ```bash
   kubectl logs -n sage -l app=outbox-worker --previous
   ```
...
```bash
# Force restart
kubectl rollout restart deployment/outbox-worker -n sage

# If pods are stuck
kubectl delete pods -n sage -l app=outbox-worker --force --grace-period=0
```

**If startup failures:**
```bash
# Check configmap
kubectl get configmap -n sage outbox-config -o yaml

# Check secrets
kubectl get secret -n sage sage-secrets -o yaml

# Verify connectivity to broker
kubectl run -it --rm debug --image=nicolaka/netshoot -n sage -- bash
# Inside pod:
...
**Rollback if needed:**
```bash
kubectl rollout undo deployment/outbox-worker -n sage
```
...
2. **Check worker health:**
   ```bash
   kubectl top pods -n sage -l app=outbox-worker
   kubectl logs -n sage -l app=outbox-worker --tail=100
   ```

3. **Check broker health:**
   ```bash
   # For Kafka
   kubectl exec -n sage kafka-0 -- kafka-broker-api-versions \
     --bootstrap-server localhost:9092
   
   # For RabbitMQ
   kubectl exec -n sage rabbitmq-0 -- rabbitmqctl status
   ```
...
```bash
# Increase replicas
kubectl scale deployment/outbox-worker -n sage --replicas=5

# Verify scaling
kubectl get hpa -n sage
```

**Optimize batch size:**
```yaml
# Update configmap
kubectl edit configmap outbox-config -n sage
...
kubectl exec -n sage kafka-0 -- kafka-topics \
  --alter --topic saga-events \
  --partitions 10 \
  --bootstrap-server localhost:9092

# Scale RabbitMQ
kubectl scale statefulset/rabbitmq -n sage --replicas=3
```

#### Monitoring
```bash
# Watch metrics
watch -n 5 'kubectl exec -n sage postgresql-0 -- psql -U sage -c \
...
3. **Check broker logs:**
   ```bash
   kubectl logs -n sage -l app=kafka --tail=200 | grep -i error
   ```
...
from sage.outbox.storage import get_storage
...
```yaml
# Update resources
kubectl patch deployment outbox-worker -n sage -p '
{
  "spec": {
    "template": {
      "spec": {
...
```bash
# 1. Stop workers
kubectl scale deployment/outbox-worker -n sage --replicas=0

# 2. Stop orchestrator
kubectl scale deployment/saga-orchestrator -n sage --replicas=0
...
kubectl scale deployment/outbox-worker -n sage --replicas=1
# Monitor for 5 minutes
kubectl scale deployment/outbox-worker -n sage --replicas=3
```
...
```bash
# Backup database
kubectl exec -n sage postgresql-0 -- pg_dump -U sage sage > backup_$(date +%Y%m%d_%H%M%S).sql

# Backup to S3
kubectl exec -n sage postgresql-0 -- pg_dump -U sage sage | \
...
cat backup_20231223_120000.sql | \
  kubectl exec -i -n sage postgresql-0 -- psql -U sage sage
```
