...
This directory contains monitoring configurations for the Sage saga orchestration system.
...
2. **Verify pod annotations:**
   ```bash
   kubectl get pods -n sage -o yaml | grep -A 5 annotations
   ```

3. **Check metrics endpoint:**
   ```bash
   kubectl port-forward -n sage pod/outbox-worker-xxx 8000:8000
