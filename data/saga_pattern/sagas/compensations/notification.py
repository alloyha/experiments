...
async def send_order_cancellation_notification(
    ...
    try:
        ...
        from sagas.actions.notification import send_order_cancellation_email
        ...
        if user_email and order_details:
            ...
            if ctx:
                ctx.logger.info(f"Sent order cancellation email to {user_email}")
    
    except Exception as e:
        # Notification failures during compensation are logged but not critical
        if ctx:
            ctx.logger.warning(f"Failed to send cancellation notification: {str(e)}")
...
async def send_payment_failure_notification(
    ...
    try:
        ...
        if user_email:
            ...
            if ctx:
                ctx.logger.info(f"Sent payment failure notification to {user_email}")
    
    except Exception as e:
        if ctx:
            ctx.logger.warning(f"Failed to send payment failure notification: {str(e)}")
...
async def send_shipping_cancellation_notification(
    ...
    try:
        ...
        if ctx:
            ctx.logger.info("Sent shipping cancellation notifications")
    
    except Exception as e:
        if ctx:
            ctx.logger.warning(f"Failed to send shipping cancellation notification: {str(e)}")
...
async def retract_bulk_notifications(
    ...
    try:
        ...
        if ctx:
            ctx.logger.info(f"Sent retraction emails to {len(successful_recipients)} recipients")
    
    except Exception as e:
        if ctx:
            ctx.logger.warning(f"Failed to send bulk notification retractions: {str(e)}")
...
async def cancel_scheduled_notifications(
    ...
    try:
        ...
        if ctx:
            ctx.logger.info(f"Cancelled scheduled notification {schedule_id}")
    
    except Exception as e:
        # Scheduled notification cancellation failure is not critical
        if ctx:
            ctx.logger.warning(f"Failed to cancel scheduled notification: {str(e)}")
...
async def suppress_notification_preferences(
    ...
    try:
        ...
        if ctx:
            ctx.logger.info(f"Restored notification preferences for user {user_id}")
    
    except Exception as e:
        if ctx:
            ctx.logger.warning(f"Failed to restore notification preferences: {str(e)}")